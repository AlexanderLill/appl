...
{
	DEBUG_TRACE( cout << "gnap: getNewAlphaPlane" << endl; );
	if(cn.isFringe())
	{
		assert(false);
		cout << "Code bug" << endl;
	...
	else
	{
		...
		FOR (a, problem->getNumActions())
		{
						...
						//FOR (o, QaXn.getNumOutcomes())
					{
						...
						if (NULL != e)
						{
							...
							*/
							...
							DEBUG_TRACE( cout << " + " << childLB * e->obsProb << " = " << sum << endl; );
			...
			DEBUG_TRACE( cout << "gnap: sum*discount=" << sum * problem->getDiscount() << " (discount=" << problem->getDiscount() << ")" << endl; );
			sum *= problem->getDiscount();

			DEBUG_TRACE( cout << "gnap: sum+immediateReward=" << sum + cn.Q[a].immediateReward << " (immRew=" << cn.Q[a].immediateReward << ")" << endl; );
			...
			DEBUG_TRACE( cout << " => NEW maxAction=" << maxAction << " with LB=" << maxActionLB << endl; );

		}
		assert(maxActionLB !=  -DBL_MAX);


        DEBUG_TRACE( cout << "gnap: maxAction: " << maxAction << " "; );
        DEBUG_TRACE( cn.print(); );
        DEBUG_TRACE( cout << endl; );
		getNewAlphaPlaneQ(result, cn, maxAction);


		DEBUG_TRACE( cout << "resulting alpha : "; );
